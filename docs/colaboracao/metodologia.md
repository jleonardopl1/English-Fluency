...
Adaptação do **AGENTS-COLLAB.md** (github.com/Rlealbarili/Agents-Collab.md) e da estruturação
multiagente do repositório irmão **agrodecision** à stack de **voz** do English Fluency. O
problema que ela resolve: **vários agentes de IA trabalham no mesmo projeto em sessões
diferentes, às vezes em paralelo, e cada um chega sem saber das decisões recentes.**
...
| Convenções | `CLAUDE.md` | Raramente | Objetivo, stack, regras, schema, comandos, fase |
| Ponteiro | `AGENTS.md` | Quase nunca | Aponta para os dois acima |
| Pedagogia | `skill/` | Por evolução do ensino | A "alma" do Alex (comportamento de coaching) |
| Detalhe | `docs/`, specs | Por área | Aprofundamento de um tópico (app, voz) |

Ordem de leitura: **`AGENTS-COLLAB.md` → `CLAUDE.md` → `docs/`/specs → `skill/` → código.**
