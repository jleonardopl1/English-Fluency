...
- **O que foi testado:** (typecheck/lint/build/voz no preview — cole o resultado, não só "passou")
...
- **Não invente "ok".** Se um teste falhou (ou a voz não conectou), diga isso e cole a saída.
- **Registre armadilhas novas** na seção *Armadilhas conhecidas* — toda pegadinha que você
  descobriu (ex.: token efêmero expirando, permissão de microfone) economiza horas do próximo.
...
Armadilhas. A pedagogia do Alex está em skill/. Ao terminar, escreva um Handoff Block no
AGENTS-COLLAB.md.
```
